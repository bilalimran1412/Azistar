...
function GetDetails() {
    ...
    const handleWebsiteNameClick = async (id) => {
        try {
            const response = await fetch(`http://localhost:4000/api/v1/scrap_data/websites/${id}`);
            if (!response.ok) {
    ...
    const handlePatternClick = (tag, pattern, response, id) => {
        setFormData({ tag, pattern, response, id });
        console.log('Form Data:', { tag, pattern, response, id }); // Debugging line
        setShowForm(true);
    };
    
    const handleUpdateData = async () => {
        console.log('Updating with ID:', formData.id); // Debugging line
        if (!formData.id) {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/v1/scrap_data/update/${formData.id}`, {
                method: 'PUT',
                headers: {
                    ...
                    <table className="data-table">
                        ...
                        <tbody>
                            {selectedWebsiteData.filter(item => item !== null).flatMap(item => {
                                ...
                                return item.patterns.map((pattern, index) => (
                                    <tr key={`${item._id}-${index}`}>
                                        <td onClick={() => handlePatternClick(item.tag, pattern, item.responses)}>
                                            {pattern}
                                        </td>
