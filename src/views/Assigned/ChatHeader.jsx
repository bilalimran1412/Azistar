...
const ChatHeader = ({ SolvedChat, messages, handleDelete, VisterBan, addMessage, userId, customerId, oldUserId }) => {
    ...
    const fetchAgentByUserId = async () => {
        try {
            ...
            // Fetch agents
            const agentsResponse = await fetch(`http://localhost:4000/api/v1/agents/${userId}`);
            ...
            // Fetch users
            const usersResponse = await fetch(`http://localhost:4000/api/v1/auth/${oldUserId}`);
    ...
    const handleAssigneeSelect = async (assigneeName, assigneeId) => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:4000/api/v1/customers/updateuser/${customerId}`, {
                method: 'PATCH',
    ...
    const handleSendTranscript = async (email) => {
        try {
            const response = await fetch('http://localhost:4000/api/v1/customers/send-transcript', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
