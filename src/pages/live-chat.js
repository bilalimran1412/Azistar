...
const LiveChat = () => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/v1/message/${userId}/${websiteId}`);
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      if (selectedCustomer) {
        try {
          const response = await axios.get(`http://localhost:4000/api/v1/message/${userId}/${websiteId}/${selectedCustomer}`);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:4000/api/v1/message/chat`, {
        message: input,
        userId,
        sender: 'user',
        websiteId,
